...
# Global def() setup
#################################################
### returns most recent date (last date) from database as string ###
def get_recent_date():
...
### takes date as a string (YYYY-MM-DD) and returns the date one year prior as string ###
def get_year_ago(DateAsString):
    recent_date_as_date = dt.datetime.strptime(DateAsString,'%Y-%m-%d').date()

    one_year_ago = recent_date_as_date - dt.timedelta(days=365)
    one_year_string = one_year_ago.strftime('%Y-%m-%d')

    return one_year_string
#################################################
# Flask Routes
#################################################
